...
open class UDPBroadcastConnection {
    ...
    var handler: ((_ ipAddress: String, _ port: Int, _ response: String?) -> Void)?
     ...
     */
    public init(port: UInt16, handler: ((_ ipAddress: String, _ port: Int, _ response: String?) -> Void)?) { // was [UInt8]
    ...
    fileprivate func createSocket() -> Bool {
        ...
        newResponseSource.setEventHandler { [unowned self] in
            ...
            let cleandata = response.filter { $0 != 0 }
            let data = Data(bytes: cleandata)
